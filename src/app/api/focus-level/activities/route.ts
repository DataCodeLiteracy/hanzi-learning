...
export async function POST(request: Request) {
  try {
    ...
    if (!token) {
      return NextResponse.json({ error: "focusLevelIdToken이 필요합니다." }, { status: 400 })
    }
    const verified = await verifyFocusLevelIdToken(token)
    if (!verified) {
      return NextResponse.json({ error: "focus-level 로그인이 유효하지 않습니다." }, { status: 401 })
    }
    const base =
    ...
    return NextResponse.json({ ...result, focusEmail: verified.email ?? null })
  } catch (error) {
    console.error("[focus-level/activities]", error)
    return NextResponse.json({ error: "활동 목록을 불러오지 못했습니다." }, { status: 500 })
  }
}
